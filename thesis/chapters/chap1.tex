\chapter{Classification of Integral Affine Closed Surfaces}\label{ch:class-IAS}
As we have seen in the previous chapter, the integral affine structure induced
...
lattice inducing the integral affine structure. We have seen in Chapter
\ref{LagrangianFibrations} that after having fixed an integral affine structure
on $B$, all Lagrangian fibrations over $B$ locally look like $T^*B/\Lambda \to
    B$ where $\Lambda \subset T^*B$ is the period lattice inducing the integral
affine structure.
...
goal of this chapter is to carry this out for compact surfaces. We will follow
closely the ideas used in \cite{Sepe2010} and \cite{MISHACHEV1996301}.
...
\end{corollary}
\begin{proof}
    As any covering space of an integral affine structure inherits an integral affine structure by Lemma \ref{lemma:pullback-affine-structure}, it follows that the
    ...
    $S^2$ and hence admits no affine structure. If $\bar{g} = 2$ we get the Klein bottle which is covered by $\mathbb{T}^2$ and hence it can admit such a structure.
\end{proof}
...
This was first obtained in \cite{Sepe2010}. The following observation will
greatly simplify the classification process.

\begin{lemma}\label{lemma:torus-complete}
...
From the above discussion, classifying complete integral affine structures on a
compact surface $M$ amounts to classifying injective homomorphisms from the
fundamental group into the group $\text{Aff}_{\mathbb{Z}}(\mathbb{R}^2)$ which
induce a free and proper action on $\mathbb{R}^2$.

Indeed, Proposition \ref{prop:complete-if-quotient} tells us that every
complete integral affine manifold is the quotient of a subgroup $\Gamma \subset
    \affz (\R^n)$ acting freely and properly on $\R^n$. If $\dev$ is a
diffeomorphism, then if $\gamma \in \pi_1(M)$ and $\hol(\gamma)$ acts trivially
on $\R^n$, equivariance implies
\begin{align*}
    \dev(\gamma \cdot x) & = \hol(\gamma)\dev(x) \\
                         & = \dev(x)
\end{align*}
and hence injectivity of $\dev$ implies $\gamma$ acts trivially.

We now wish to do this for the torus. We will establish some lemmas to help us
determine when an integral affine transformation acts freely on $\R^2$.
...
\begin{restatable}{theorem}{classificationtorus}
    \label{thm:classification-torus}
    Any integral affine structure on $\mathbb{T}^2$ has its affine holonomy group generated by one of the following subgroups of $\affz(\mathbb{R}^2)$
    \begin{enumerate}
...
\begin{proof}
    ...
    If both $C= \left( 1 ,\begin{pmatrix}
            ...
            \end{pmatrix}\right)$ and $D =\left(1, \begin{pmatrix}
                b \\ d
            \end{pmatrix}\right)$ are pure translations, i.e. the linear component is the identity, injectivity forces that
...
still have to tackle the question of when two lattices give the same integral
affine structure. This occurs when the subgroups of the integral affine group
determined by the holonomy representation are conjugate to each other by Lemma
\ref{lemma:equivalent-dev-pair}.
...
\begin{restatable}{theorem}{torusA}\label{thm:torus-series-a}
...
\begin{proof}
    ...
    $$
        ...
        \end{pmatrix}
    $$
    where $x \in \mathbb{R}$ is arbitrary depending on $\bar{h}$. Now Equation \ref{eq:vector-side} becomes
    $$
        \begin{pmatrix}
            x \\ 0
...
\end{lemma}
\begin{proof}
    We can pull back the integral affine structure on $N$ to $M$. A regular covering induces an injection of fundamental groups $\iota: \pi_1(M) \to \pi_1(N)$. We can induce the holonomy representation on $M$ by $\hol \circ \iota$ where $\hol$ is the affine holonomy representation induced by the
    integral affine structure on $N$. As $M$ and $N$ share a common universal cover $X$, the integral affine structure on $N$ induces a developing map $\dev : X \to \R^n$ and this induced holonomy representation will remain equivariant with respect to this developing map as the action of $\pi_1(\mathbb{T}^2)$ on the universal cover is a restriction of the action of $\pi_1(K^2)$.
    ...
    \begin{align*}
        ABA & = \begin{pmatrix}
            ...
            & = \begin{pmatrix}
                ...
                \end{pmatrix}.
    \end{align*}
    Comparing entries yields that $a(b+c) = 0$ and $d(b+c) = 0$. However, the trace condition implies that $a$ and $d$ can't both be zero and so $b = -c$.
...
\begin{proof}
    \begin{case}[$A=I$, $B=B_1$]
        ...
        $$(2k+1)y_1 + z_1 = z_1$$
        which gives $(2k+1)y_1 = 0$. But now $y_1>0$ and $2k+1 = 0 \implies k = -\frac{1}{2} \notin \Z$ and so this action can have no fixed points.

        The integral affine structure on $K^2$ is thus given by $$a = \left(I, \begin{pmatrix}
                    0 \\ x_2
                \end{pmatrix}\right), \quad b = \left(B, \begin{pmatrix}
                    y_1 \\ 0
                \end{pmatrix}\right), \quad x_2, y_1>0$$
        and the induced integral affine structure on the torus is given by
        $$
    ...
    \begin{case}[$A=A_1$, $B=B_1$]
        Let $a = \left( A_1, \begin{pmatrix}
                ...
                \end{pmatrix} \right)$ and $b = \left( B, \begin{pmatrix}
                    y_1 \\ y_2
                \end{pmatrix} \right)$.
        As $A_1^2 \neq I$ and $B^2 = I$, by the torus classification such a structure on $K^2$ would need to induce the second family from the torus classification in Theorem \ref{thm:classification-torus}.
        By this consideration, we obtain that $x_1 = 0$. We calculate the group law $aba = b$.
        This gives us
        \begin{align*}
            aba & = \left(B, \begin{pmatrix}
                ...
                & = \left( B, \begin{pmatrix}
                              ...
                              \end{pmatrix} \right)
        \end{align*}
        which imposes the restriction $n(y_2 - x_2) = 0$ and since $n \neq 0$ this implies $y_2 = x_2$. We can also assume that $x_2>0$ by conjugating the generators by $(-I, \bar{0})$ and that $y_1>0$ by changing generators $b \mapsto b^{-1}$ if necessary.
        ...
        \begin{align*}
            a^qb^k = \left( \begin{pmatrix}
                            ...
                            \end{pmatrix}, \begin{pmatrix}
                                           ...
                                           \end{pmatrix} \right)
        \end{align*}
        and calculating this action on $z = \begin{pmatrix}
                z_1 \\ z_2
            \end{pmatrix}$ yields
        ...
        $$a^q \circ z = \begin{pmatrix}
                * \\
                z_2 + qx_2
            \end{pmatrix}$$
        and so we require that $qx_2 =0$ but $x_2>0$. Therefore, the only element with a fixed point occurs when $k=q=0$ which is the identity and hence the action is free.

        The integral affine structure on $K^2$ is $$a = \left(A_1, \begin{pmatrix}
                    0 \\ x_2
                \end{pmatrix}\right), \quad b = \left(B_1, \begin{pmatrix}
                    y_1 \\ x_2
                \end{pmatrix}\right), \quad x_2, y_1>0$$

        and the induced integral affine structure on $\mathbb{T}^2$ is given by
...
\begin{proof}
    \begin{case}[$A=I$, $B=B_2$]
        ...
        We will do the same for the translational component of $b$ and assume $y_2 = 0$ and $y_1>0$ after relabeling $-y_2 -y_1 \to y_1$. This finishes our generating set which we write as
        $$
            a = \left(I , \begin{pmatrix}
                    x_1 \\ -x_1
                \end{pmatrix}\right), \quad b =\left(B , \begin{pmatrix}
        ...
        but this action is by translations and hence is free and so $A=I, B=B_2$ does induce an integral affine structure.

        The integral affine structure on $K^2$ is given by $$a = \left(I, \begin{pmatrix}
                    x_1 \\ -x_1
                \end{pmatrix}\right), \quad b = \left(B_2, \begin{pmatrix}
                    y_1 \\ 0
                \end{pmatrix}\right), \quad x_1, y_1>0$$
        and the induced structure on the torus is given by
        $$
            a = \left(I , \begin{pmatrix}
                    x_1 \\ -x_1
                \end{pmatrix}\right), \quad b^2 =\left(I , \begin{pmatrix}
    ...
    \begin{case}[$A=A_4$, $B=B_2$]
        ...
        $$
            \left(\begin{pmatrix}
                ...
                \end{pmatrix}\right)
        $$
        and so $ny_2 + (1-n)x_2 = 0 \implies y_2 = \frac{n-1}{n}x_2$.

        Our generators for the integral affine structure on $K^2$ are therefore $$ a =
            \left(\begin{pmatrix}
                    1 & n \\
                    0 & 1
                \end{pmatrix} , \begin{pmatrix}
        ...
        In fact, we shall now show that this is sufficient. We calculate that $$ a^k =
            \left(\begin{pmatrix}
                    1 & kn \\
                    0 & 1
                \end{pmatrix} , \begin{pmatrix}
    ...
    \end{case}
\end{proof}
This completes the classification of the integral affine structures on the Klein bottle. We summarize the results of the previous two lemmas into the following theorem.
\begin{restatable}{theorem}{IASKlein}\label{thm:IAS-Klein}
    The possible generators for the holonomy representation of the integral affine structures on the Klein bottle are
    \begin{enumerate}
...
bottle, i.e. they preserves some Riemannian metric as we have that the group
...
\subsection{Fundamental domains for the action of the holonomy group on $\R^2$.}
\begin{figure}[H]
    \centering
    \incfig[0.4]{secondlatticeguy}
    \caption{Fundamental domain for the torus lattice $\mathcal{A}_{a,b,c,d}$.}
\end{figure}

\begin{figure}[H]
    \centering
    \incfig[0.4]{firstlattice}
    \caption{Fundamental domain for torus lattice $\mathcal{B}_{1,1,1}$.}
\end{figure}

\begin{figure}[H]
    \centering
    \incfig[0.5]{KleinTilingsProper}
    \caption{Fundamental domain for a Klein bottle lattice from family 1) with $x_2 = y_1= 1$.}
\end{figure}