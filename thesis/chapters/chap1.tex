...
flat connection on closed orientable surfaces. We can handloe the
non-orientable case by applying the following classification theorem.
\begin{theorem}
...
\begin{proof}
    cite "On the existence of a connection with zero curvature" by Milnor.
\end{proof}
\begin{corollary}
...
\begin{proof}
    This follows from the classification of closed 2 surfaces. (TODO: get source - probably hatcher or something)
\end{proof}
...
\begin{proof}
    Suppose there was such an integral affine structure. Then
...
\begin{proof}
    ...
    orientable double cover of a non-orientable integral affine surface must be $\mathbb{T}^2$. Furthermore, it must have non-orientable genus (TODO: define this somewhere?) $\bar{g}= 1 \text{ or } 2$. If $\bar{g}=1$ our surface is homeomorphic to $\mathbb{R}P^2$ which is covered by
    $S^2$ and hence admits no such structure. If $\bar{g} = 2$ we get the Klein bottle which is covered by $\mathbb{T}^2$ and hence can admit such a structure.
\end{proof}
...
\begin{proof}
    ...
    our linear holonomy group $\Gamma \subseteq \text{SL}(2, \mathbb{Z})$ by (TODO: Reference Corr 0.26). But now by (TODO: finish nilpotent section e.g. combine to theorem).
\end{proof}
...
surface $M$ amounts to classifying injective homomorphisms from the fundamental
group into the group $\text{Aff}_{\mathbb{Z}}(\mathbb{R}^2)$ which induce a
free and proper action on $\mathbb{R}^2$.
...
\end{lemma}
\begin{proof}
    By our previous lemma, $A$ must have $1$ as an eigenvalue. We can take an eigenvector $e_1$ such that $e_1$ is integral, e.g. its components are integers.
...
\begin{proof}
    ...
    $\tilde{A}$ is the identity this must also be a translation e.g. $\tilde{A}'^k
...
$\mathcal{A}_{a,b,c,d}$ induce a Euclidean structure on the torus - e.g. their
...
\begin{proof}
    ...
    affine manifold with nilpotent fundamental group and hence by (TODO: reference
    theorem) is complete. It follows that any integral affine structure on $N$ must
    also be complete.
\end{proof}
...
\begin{remark}
    Namely, this implies that once we classify the injective homomorphisms from $\Gamma \to \affz(\R^2)$, we can easily check
...
representation where $A,B \in GL(2, \Z)$ and $\bar{x}, \bar{y} \in \mathbb{R}$.
...
\begin{proof}
    Let $A= \begin{pmatrix}
            a & b \\
            c & d
        \end{pmatrix} \in GL(2, \Z) $. Again we m ust have that $d = 2-a$ by the trace condition. Calculating the group law $ABA =B$ yields
    \begin{align*}
        ABA & = \begin{pmatrix}
                    ab + ac  & b^2 + ad \\
                    ad + c^2 & bd + cd
...
bottle, e.g. the preserves some Riemannian metric as we have that they group
...
some pictures of some of the different families of integral affine structures
on both the torus and the Klein bottle. (TODO: Lattices for the Klein Bottle)
