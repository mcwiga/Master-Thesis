...
\begin{proof}
    ...
    \begin{center}
        ...
        $= \text{dev}\circ \gamma \circ dev^{-1} = \text{hol}(\gamma)dev\circ dev^{-1} = \text{hol}(\gamma) \in \Gamma$
    \end{center}
...
\begin{proof}
    ...
    Let $q: E \mapsto E/E_0$ be the canonical projection. Denote by $R$ the radiant vector field on $E/E_0$. If $\{y_1,\dots , y_m\}$ are coordinates
...
\begin{proof}
    ...
    Repeating this process we eventeually get a curve that develops entirely in
    $F_0 = \{0\}$ showing injectivity.
\end{proof}
\section{Symplectic Geometry and Integral Affine Manifolds}

In this section we will recall some of the basics of symplectic geometry and
then formulate some results relating to integral affine manifolds.
