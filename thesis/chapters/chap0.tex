...
\begin{definition}[Radiant Manifold]
    An affine manifold is said to be radiant if its holonomy representation has a fixed point.
\end{definition}
...
\begin{proof}
    ...
    We now choose a developing map $dev: \tilde{M} \mapsto \mathbb{R}^n$ such that $dev(\tilde{x_0}) = 0$. Let $hol$ be the holonomy representation
    and $hol(\pi_1(M)) = \Gamma$ be the affine holonomy group.
    ...
    \textbf{Claim}: We can construct vector fields $X_i$ on  M, tangent to $\mathfrak{F}_i$ such that $X_i(l_i)=1$ using
    partitions of unity. These lift to vector fields $\tilde{X}_i$ on the universal cover.
    ...
    \begin{center}
        $\frac{d}{dt}dev \circ \gamma_i(t) = dev_* \circ \tilde{X}(\gamma_i(t))$\\
        $\implies dev \circ \gamma_i(t) = dev_* \circ \tilde{X}_i(\gamma_i(t))t + dev \circ \gamma_i(0)$
    \end{center}

    applying our linear functional $l_i$ to this we obtained
    \[l_i(dev \circ \gamma_i(t)) = l_i \circ dev_*(\tilde{X}_i(\gamma_i(t)))t + l_i(dev \circ \gamma_i(0))\]
    \[= l_i(\tilde{X}_i)t + l_i(dev \circ \gamma_i(0)) = t + l_i(dev \circ \gamma_i(0)) \tag{*} \label{eq:flow}\]

    Now we can start our inductive step. Taking $i=n$ we start at $\tilde{x}_0 \in
        \tilde{M}$ such that $dev(\tilde{x}_0) = 0$. We flow along $\tilde{X}_n$ for
    time $t=l_n(v)$ until the point $\tilde{x}_1 \in \tilde{M}$ with
    $dev(\tilde{x}_1) = v_1$. From \eqref{eq:flow} we obtain

    \[l_n(v_1) = l_n(v) + 0\]
    \[\implies v- v_1 \in \ker l_n = F_{n-1}\]

    Now, we continue e.g. flow along $\tilde{X}_{n-1}$ for $t=l_{n-1}(v-v_1)$
    ending at $\tilde{x}_2$ with $dev(\tilde{x}_2) = v_2$. Then
    ...
    \[dev \circ \gamma_t(s) = dev(\gamma_0(s)) -tl_n(\delta(s))dev_*(\tilde{X}(\gamma_0(s)))\]
    \[\implies l_n(dev \circ \gamma_t(s)) = l_n(dev(\gamma_0(s))) -tl_n(\delta(s))l_n \circ dev_*(\tilde{X}(\gamma_0(s))) \]
    \[ \implies l_n(dev \circ \gamma_t(s)) = l_n(dev(\gamma_0(s))) - tl_n(\delta(s))\]

    At $t=1$ this yields that $dev \circ \gamma_t(s) \in \ker l_n = F_{n-1}$.
