...
A consequence of this is the following lemma. (TODO: WTH is group homology?
Should say some stuff about these things.)
\begin{lemma}
...
The following lemma gives some insight on why this is a useful idea. (TODO:
check this lemma)
\begin{lemma}
...
\begin{proof}
    ...
    As $H^0(G, E/E_U)$ is the set of stationary points of the $G$-module $E/E_U$, assuming $H^0(G, E/E_U)\neq0$
...
\begin{proof}
    ...
    This is equivalent by (TODO: define group group cohomology) Lemma \ref{lemma:nilpotent-group-coho} and Lemma \ref{lemma:nilpotent-group-equiv-hom-coho}
    to $H^{0}(G, \Hom(E/E_U)) = 0$.

    To this end we suppose that $R: E/E_U \mapsto E$ is fixed under all $\gamma(g)$
    for $g\in G$. Then, letting $d=\dim(E_U)$ for any $d$ elements $g_1, \dots, g_d
        \in G$ we have
    \begin{center}
    ...
    showing that $E/E_U$ is spanned by vectors of the form $(1 - \beta(g))x$ with $x \in E/E_U$. By definition, this is equivalent to
...
\begin{prop}
    ...
    canonical projection map.
\end{prop}
\begin{proof}(TODO: format this better)
...
\begin{corollary}
    Let $V$ be a vector space. If $\pi : \mathfrak{g} \mapsto \mathfrak{gl}(V)$
...
\begin{lemma}\label{lemma:exists-map}
    ...
    contain an expansion of $E$. Then for each $n \in \mathbb{N}_{>0}$, there is a $C^r$ map $\varphi : E \mapsto \mathbb{R}$ for any $r \in \Z_{>0}$
...
\begin{proof}
    ...
    We now choose a developing map $\dev: \tilde{M} \to \mathbb{R}^n$ such that $\text{dev}(\tilde{x_0}) = 0$. Let $\hol$ be the holonomy representation
    and $\text{hol}(\pi_1(M)) = \Gamma$ be the affine holonomy group.

    Now, suppose that the linear affine holonomy group is unipotent. Then there
    exists a linear flag
    \begin{center}
        $0 = F_0 \subset F_1 \subset \dots \subset F_n = \mathbb{R}^n$
    \end{center}
    preserved by the linear affine holonomy group as shown in Section 17.5 of \cite{humphreys2012linear}. Furthermore, the induced action on the quotients $F_k/F_{k-1}$ is trivial and so
    the restriction to $F_k$ preserves a linear functional $l_k : F_k \to \mathbb{R}$ with kernel $F_{k-1}$.
    ...
    linear map $l_i$ determines a parallel one-form $\omega_i$ defined on
